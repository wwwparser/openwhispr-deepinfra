...
## Текущий статус: стабильно работают OpenRouter и Groq

На практике, на реальной конфигурации (тихий USB-микрофон, WebM/Opus от
MediaRecorder), стабильнее и быстрее всего показали себя провайдеры **OpenRouter**
и **Groq** (модель `whisper-large-v3-turbo`). Сейчас приложение работает на
**OpenRouter**: он распознаёт даже тихое аудио за **0.3–0.7 секунды**, без пустых
ответов и без каскада повторов. **Groq тоже работает хорошо** — быстро и надёжно
(именно на нём приложение стабильно работало изначально).

**DeepInfra запустить надёжно не удалось** так, чтобы «просто работало»: turbo-модель
периодически отдаёт пустой `{"text":""}` и страдает от холодных стартов (10–15 с)
— несмотря на все доработки ниже (конвертация в WAV, speechnorm, прокси через
main-процесс, keep-warm, авто-откат на large-v3). Эти меры смягчают проблему, но
не убирают её полностью на стороне DeepInfra.

Поэтому **сейчас приложение по умолчанию работает на OpenRouter** (а Groq — как
проверенная и одинаково надёжная альтернатива). Вся интеграция DeepInfra
(описанная ниже) остаётся в коде как опциональный провайдер — её можно включить
в настройках, если у DeepInfra улучшится стабильность инференса.
